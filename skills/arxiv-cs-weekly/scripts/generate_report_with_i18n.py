#!/usr/bin/env python3
"""
ArXiv CS Weekly - 完整版（含翻译和科普）
生成包含论文摘要、中文翻译和科普描述的周报
"""
...
def get_keywords(title):
    ...
    maps = {
        ...
        'video': '视频', 'robot': '机器人', 'reinforcement': '强化学习',
        ...
        'test-time': '测试时', 'training': '训练', 'inference': '推理'
    }
...
def generate_report_with_translations():
    """生成报告，包含需要翻译和科普的标记"""
    html = fetch_page()
    if not html:
        return None, []
    ...
    # 取前8篇（减少数量以控制整体长度）
    display_count = min(8, len(papers))
    papers_to_process = []
    ...
    for i, p in enumerate(papers[:display_count], 1):
        ...
        # 获取摘要
        print(f"正在获取论文 {i}/{display_count}: {aid}", file=os.sys.stderr)
        ...
        # 英文摘要
        if abstract:
            lines.append(f"📝 **英文摘要**：{abstract[:400]}{'...' if len(abstract) > 400 else ''}")
            lines.append("")
        
        # 标记需要翻译和科普（由 agent 在发送前处理）
        lines.append(f"[TRANSLATE_AND_EXPLAIN: {aid}]")
        lines.append("")
        lines.append("---")
        lines.append("")
    ...
    if len(papers) > display_count:
        ...
        for i, p in enumerate(papers[display_count:display_count+10], display_count+1):
            title = p.get('title', '未知')[:45] + "..." if len(p.get('title', '')) > 45 else p.get('title', '未知')
            aid = p.get('arxiv_id', '')
            subj = simplify_subject(p.get('subjects', ''))
            is_new = "🆕 " if aid not in prev_ids else ""
    ...
    lines.append("---")
    lines.append("")
    lines.append("🤖 *OpenClaw 自动生成* | 回复 \"解读第N篇\" 获取深度技术分析")
    
    save_state(current_ids)
    return "\n".join(lines), papers_to_process

if __name__ == "__main__":
    report, papers = generate_report_with_translations()
    if report:
        print(report)
        print("\n" + "="*60, file=os.sys.stderr)
        print(f"周报基础内容生成完成！共 {len(papers)} 篇论文", file=os.sys.stderr)
        print("注意：每篇论文的 [TRANSLATE_AND_EXPLAIN] 标记需要替换为中文翻译和科普描述", file=os.sys.stderr)
